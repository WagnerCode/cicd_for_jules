...
| **`CLUSTER_GATEWAY`**     | `ci/variables.yml`         | `10.79.11.17`                                                                                                                                                                                   | The default gateway inside the cluster subnet. Passed into the Terraform/OpenTofu templates to build routes for the VMs.                                                                                |
...
| *JAM Stage Vars*          | GitLab CI/CD Settings      | *None*                                                                                                                                                                                          | The `jam` stage requires monitoring variables: `project_id`, `log_group_id`, `sa_journal_api_key`, `sa_audit_api_key`, `vmagent_service_clientID`, and `vmagent_service_clientSecret`. These must be set in the GitLab CI/CD settings.      |

Additional Terraform inputs: the `terraform:apply` job reads cloud.ru credentials and network parameters that are **not** defined with defaults. Set `CLOUDRU_PROJECT_ID`, `CLOUDRU_KEY_ID`, `CLOUDRU_SECRET`, `GIS_PROJECT_NAME`, `CLUSTER_NUMBER`, `USERS_SUBNET`, `INFRA_SUBNET_GITLAB`, `INFRA_SUBNET_JUMPHOST`, `USER_NAME`, `USER_PASS`, and (if it cannot be generated from `SSH_PRIVATE_KEY`) `USER_PUBLIC_KEY` as GitLab CI/CD variables so `terraform-ci/scripts/generate_tfvars.py` can render `terraform.tfvars`.
...
*   **Input**:
    ...
    *   Requires cloud provider credentials and network parameters (`CLOUDRU_PROJECT_ID`, `CLOUDRU_KEY_ID`, `CLOUDRU_SECRET`, `GIS_PROJECT_NAME`, `CLUSTER_NUMBER`, `CLUSTER_SUBNET`, `CLUSTER_GATEWAY`, `USERS_SUBNET`, `INFRA_SUBNET_GITLAB`, `INFRA_SUBNET_JUMPHOST`, `USER_NAME`, `USER_PASS`, optionally `USER_PUBLIC_KEY`) to be available as environment variables.
*   **Process**:
...
*   **Input**:
    ...
    *   Requires monitoring-related variables in the GitLab CI/CD settings: `project_id`, `log_group_id`, `sa_journal_api_key`, `sa_audit_api_key`, `vmagent_service_clientID`, and `vmagent_service_clientSecret`.
*   **Process**:
